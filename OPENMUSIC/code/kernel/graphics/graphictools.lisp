...
(defun init-om-color-vars ()
  ...
  (setf *workspace-color* (om-make-color  0.8039216 0.79375905 0.79375905))
    ...
    (om-init-motion-click self 
                         ...
                         :motion-draw #'(lambda (view pp1 pp2)
                                          (let* ((p1 (om-view-position boxframe))
                                              ...
                                              (om-with-fg-color view *om-blue-color* ;(om-make-color-alpha 0.5 0.5 0.5 0.7)
    ...
    (when (enabled self)
      (cond 
       ((and (patchpanel-p (om-view-container self)) ok)
        ...
        t)
       ((not (patchpanel-p (om-view-container self)))
