...
(defun find-type-by-loading (pathname)
  (let ((rec *om-current-persistent*)
    ...
    (handler-bind ((error #'(lambda (c)
      ...
      (pushr (print (format nil "Type of '~A' could not be found... File will be loaded for contents checking." (namestring pathname))) 
             *import-log*)
      ...
      (setf rep 
            (cond ((and (member :omsheet *features*) (subtypep (type-of *om-current-persistent*) 'OMSheet)) :OMSH)
                  ...
                  (t nil)))
      (if rep 
          (pushr (print (format nil "Type of '~A': ~A" (namestring pathname) (string rep))) *import-log*)
        (pushr (print (format nil "Type of '~A'  definitely not found." (namestring pathname))) *import-log*))
      )
    (setf *om-current-persistent* rec)
    ;(setf *skip-libs* skip-libs)
  ...
  (let ((rep :????))
    (if (file-has-comments pathname) 
     (if (file-has-old-comments pathname)
         (progn
           (print "=============================================")
           (pushr (print (format nil "File '~A' has old header format.." (namestring pathname))) *import-log*)
           (setf rep (read-old-type pathname)))
       (setf rep (second (get-pathname-wsparams pathname))))
      (progn 
        (print "=============================================")
        (pushr (print (format nil "File '~A' has no header.." (namestring pathname))) *import-log*)))
...
(defun excluded-special-file-p (path)
  (let ((name (pathname-name path)))
    (or 
     ;;; special Dropbox 
     (and (= (length name) 5) (string-equal (subseq name 0 4) "Icon"))     
     )))

(defun object-from-file (pathname)
  (unless (excluded-special-file-p pathname)
    (let ((pathtype (catch 'check-error (file-type pathname)))
      ...
      (cond
       ...
       ((equal pathtype :EMPTY)
        (pushr (print (format nil "!!! Element '~A' could not be loaded. File is EMPTY !" (namestring pathname))) *import-log*)
       ...
       ((equal pathtype :ERROR)
        ...
        (pushr (print (format nil "!!! Element '~A' could not be loaded. Error in File !" (namestring pathname))) *import-log*))
       ((equal pathtype :FOLD)
      ; a folder params file was caught as a document
        (pushr (print (format nil "Element '~A' is an old folder params file. File will be removed." (namestring pathname))) *import-log*)
        (om-delete-file pathname))
       (pathtype (setf newobj (ws-load-more-element pathtype pathname name)))
       (t (pushr (print (format nil "!!! Element '~A' could not be loaded" (namestring pathname))) *import-log*)
