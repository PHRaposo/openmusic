...
(defmethod get-slot-in-out-names ((self MidiControl))
  (values '("self" "ctrltype" "Ldates" "ref" "port" "chan" "Lvalues")
          ...
          '(nil (( 1 (("-- Tempo (bpm)" "Tempo")
                      ("-- KeyPress" "KeyPress")
                      ("-- ChanPress" "ChanPress")
                      ("-- PitchBend (0 to 127)" "PitchBend")
                      ("-- PitchWheel (0 to 16383)" "PitchWheel")
        ...
        (loop for event in evtList do
              (cond 
               ...
               ((equal eventtype :PitchBend) 
                (setf curr-val (msb-lsb2value (second (ev-fields event)) (first (ev-fields event))))
                ; (- (msb-lsb2value (second (ev-fields event)) (first (ev-fields event))) 8192))
                ;(case control 
