...
(defmethod get-help-list ((self soundeditor))
  (list '(("tab" "Change Selection Mode")
          #+macosx("cmd+clic" "Add Marker")
...
(defmethod handle-key-event ((self soundPanel) char)
   (case char
     ...
     (:om-key-tab (change-cursor-mode self))
     (:om-key-esc 
...
(defmethod change-cursor-mode ((self soundPanel) &optional mode)
  (let* ((editor (om-view-container self))
         (controls (fourth (om-subviews editor)))
         (norm (nth 7 (oa::item-subviews controls)))
         (int (nth 8 (oa::item-subviews controls))))
    (cond ((equal (cursor-mode self) :normal)
           (setf (cursor-mode self) :interval)
           (setf (selected-p norm) nil)
           (setf (selected-p int) t))
          ((equal (cursor-mode self) :interval)
           (setf (cursor-mode self) :normal)
           (setf (selected-p norm) t)
           (setf (selected-p int) nil))
          (t nil))
    (editor-stop (editor self))
    (update-editor-controls (om-view-container self))
        ...
        (let* ((sec (nth (car (selection? self)) markers)))
          ...
          (cond 
           ((and (om-shift-key-p) (om-command-key-p))
                (setf (nth (car (selection? self)) markers) (+ 1 sec)))
           ((om-shift-key-p)
                (setf (nth (car (selection? self)) markers) (+ 0.1 sec)))
           (t (setf (nth (car (selection? self)) markers) (+ 0.01 sec)))))
          (when (> (nth (car (selection? self)) markers) max)
            (setf (nth (car (selection? self)) markers) max))
          (om-invalidate-view self t)
          (report-modifications (editor self))))))
    ...
    (let* ((thesound (object (om-view-container self)))
      ...
      (when (selection? self)
        (let* ((sec (nth (car (selection? self)) markers)))
          ...
          (cond 
           ((and (om-shift-key-p) (om-command-key-p))
            (setf (nth (car (selection? self)) markers) (- sec 1)))
           ((om-shift-key-p)
            (setf (nth (car (selection? self)) markers) (- sec 0.1)))
           (t (setf (nth (car (selection? self)) markers) (- sec 0.01)))))
          (when (< (nth (car (selection? self)) markers) 0)
            (setf (nth (car (selection? self)) markers) 0))
          (om-invalidate-view self t)
          (report-modifications (editor self))))))
