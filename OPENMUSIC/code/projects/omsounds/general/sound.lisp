...
;(ratio (round (om-sound-n-samples self) 2000)))) pour un ratio variable. 2000 car nbpix d'un ecran environ
;Bien pour les petits fichiers mais mauvais des que trop grand car bascule trop vite sur la lecture fichier
...
(defmethod update-if-editor ((self OMaiffFilebox))
  (when (editorFrame self)
    ...
    (setf (editorframe self) nil));needed in case lost sf
    (call-next-method))
