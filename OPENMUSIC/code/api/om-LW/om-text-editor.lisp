...
(defun om-zoom-pane-font-shortcut-in (pane x y spec)
  (declare (ignore x y spec))
  (apply-pane-font-step pane 1 nil))

(defun om-zoom-pane-font-shortcut-out (pane x y spec)
  (declare (ignore x y spec))
  (apply-pane-font-step pane -1 nil))

(defun om-zoom-pane-font-shortcut-reset (pane x y spec)
  (declare (ignore x y spec))
  (apply-pane-font-step pane 0 t))

;; ZOOM-INPUT: input-model entries prepended by customize-text-editor-pane.

(defun om-zoom-pane-font-input-entries ()
  ...
  `(((:gesture-spec #\= #+macosx :hyper #-macosx :control) om-zoom-pane-font-shortcut-in)
    ((:gesture-spec #\+ #+macosx :hyper #-macosx :control) om-zoom-pane-font-shortcut-in)
    ((:gesture-spec #\- #+macosx :hyper #-macosx :control) om-zoom-pane-font-shortcut-out)
    ((:gesture-spec #\0 #+macosx :hyper #-macosx :control) om-zoom-pane-font-shortcut-reset)
...
(defmethod editor-window-input-model ((self om-lisp-edit-window))
  (om-zoom-pane-font-input-entries))
  ...
  (let ((ep (om-lisp::ep win)))
    (when ep
      ...
      (setf (capi:capi-object-property ep :om-zoom-default-font)
            (ensure-font-description (capi::simple-pane-font ep)))
      (setf (capi:output-pane-input-model ep)
            (append (om-zoom-pane-font-input-entries)
